...
CHUNK_SIZE = 5  # Process 5 pages at a time (reduced from 10)
MAX_TOKENS_PER_CHUNK = 4000  # Reduced token limit
...
def process_chunk_pyq_only(client: Mistral, chunk_content: str, chunk_number: int) -> List[PYQQuestion]:
    ...
    try:
        ...
        for i, item in enumerate(questions_data):
            try:
                ...
                # Validate that this is an original PYQ (not test series)
                if is_original_pyq(question):
                    valid_questions.append(question)
                    print(f"[Chunk {chunk_number}] ✅ Valid PYQ {question.question_number}: {question.question_text[:50]}...")
                else:
...
def extract_questions_with_regex(content: str, chunk_number: int) -> List[PYQQuestion]:
    ...
    # Look for question patterns - more specific patterns
    question_patterns = [
    ...
    for pattern in question_patterns:
        ...
        for match in matches:
            try:
                ...
                # Skip if it looks like test series
                if any(pattern in question_text.lower() for pattern in ['visionias', 'sandhan', 'pt 365', 'test series', 'consider the following statements regarding']):
                    continue
...
def is_original_pyq(question: PYQQuestion) -> bool:
    ...
    # Check for test series patterns (should not be present in original PYQs)
...
def extract_pyq_only_chunked(pdf_path: str, max_pages: Optional[int] = None) -> List[PYQQuestion]:
    ...
    for i in range(0, len(pages_to_process), CHUNK_SIZE):
        ...
        if chunk_content.strip():
            chunk_questions = process_chunk_pyq_only(client, chunk_content, chunk_number)
